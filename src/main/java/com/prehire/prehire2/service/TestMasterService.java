...
public class TestMasterService {
    ...
    public TestMaster getTestById(UUID id) {
        return testMasterRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Test not found: " + id));
    }

    public List<TestMaster> getTestsByTenant(UUID tenantId) {
        return testMasterRepository.findByTenant_Id(tenantId);
    }

    public List<TestMaster> getTestsByJob(UUID jobId) {
        return testMasterRepository.findByJob_Id(jobId);
    }

    public TestMaster getTestByJobAndName(UUID jobId, String testName) {
    ...
    public List<TestMaster> getTestsCreatedBy(UUID createdById) {
